...
# --- FUNGSI UTAMA ---
def init_chatbot():
    # ... (Isi fungsi ini sama, hanya perlu menyesuaikan path folder)
    try:
    ...
    my_generation_config = {
        "temperature": 0.5, "max_output_tokens": 4096, "top_p": 0.6
    }
    my_safety_settings = {
    ...
    # Jalankan proses sinkronisasi database setiap kali aplikasi dimulai
...
if __name__ == "__main__":
    st.title("🤖 Chatbot RAG Cerdas (Multi-Dokumen)")
    
    st.info("Letakkan file-file .txt Anda di dalam folder 'sumber_dokumen' dan restart aplikasi untuk mengindeksnya.")
    ...
    if st.session_state.chatbot:
        ...
        for author, text in st.session_state.chatbot.history:
        ...
        # Input dari pengguna
        if prompt := st.chat_input("Tanyakan sesuatu dari dokumen Anda..."):
            with st.chat_message("user"):
                st.markdown(prompt)

            with st.spinner("Mencari dan merangkum jawaban..."):
                response = st.session_state.chatbot.get_response(prompt)
                with st.chat_message("assistant"):
                    st.markdown(response)
    else:
        st.error("Gagal menginisialisasi chatbot. Periksa konfigurasi API Key Anda.")